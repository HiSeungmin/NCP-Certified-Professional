...
[전통적인 방식]
전통적인 방식은 트래픽 유무와 관계없이 서버(VM/컨테이너)를 상시 기동 상태로 유지해야 하므로, 유휴 시간에도 컴퓨팅 자원 비용이 발생함. 또한 트래픽 증가에 대비한 스케일링 설계(오토스케일링, 로드밸런서 등)를 직접 구성해야 함.

    ![alt text](image/image1.png)

[서버리스 방식]
서버리스는 인프라를 사용자가 직접 프로비저닝하지 않고, 클라우드 제공자가 요청 단위로 컨테이너를 생성·실행·소멸시킴(이벤트 발생시에만 자원 할당 및 요금 지불)
